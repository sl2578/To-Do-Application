...
def index():
    ...
    tasks = [dict(id=row[0], title=row[1], descr=row[2], done=row[3], dline=row[4], finish=row[5]) for row in cur.fetchall()]
    return render_template('index.html', tasks=tasks)

# Adds new task to list and stores relevant information
@app.route('/add', methods=['POST'])
def add_entry():
    g.db.execute('insert into entries (title, descr, done, dline, finish) values (?, ?, ?, ?, ?)', [request.form['title'], request.form['descr'], 0, request.form['dline'], str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M'))])
    g.db.commit()
